...
# field={'Form name':'SMART ESTATE ANALYTICS', 'Username':'Nombre de usuario', 'Password':'Contraseña', 'Login':'Iniciar sesión'}
name, authentication_status, username = authenticator.login("SMART ESTATE ANALYTICS", "main")
    ...
    if selected=="Análisis de mercado":
        ...
        with center_aux:
            if selected_option=="Provincias":
                ...
                selected_geo = st.selectbox('**Selecciona una província:**', prov_names, index= prov_names.index(maestro_mun[maestro_mun["Municipi"]==mun_solar]["Província"].values[0]))      
        ...
        if selected_option=="Provincias":
            ...
            if selected_index=="Superficie":
                ...
                with center:
                    st.metric(label="**Superfície de viviendas de segunda mano** (m\u00b2)", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superficie media de viviendas de segunda mano", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superficie media de viviendas de segunda mano", "var")}%""")
                with right:
                    st.metric(label="**Superfície de viviendas nuevas** (m\u00b2)", value=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superficie media de viviendas nuevas", "level"):,.0f}""", delta=f"""{indicator_year(table_province_y, table_province, str(selected_year_n), "Superficie media de viviendas nuevas", "var")}%""") 
                st.markdown("")
                st.markdown("")
                # st.subheader("**DADES TRIMESTRALS MÉS RECENTS**")
                st.markdown(table_trim(table_province, 2020, True, False).to_html(), unsafe_allow_html=True)
        ...
        if selected_option=="Distritos de Barcelona":
            ...
            if selected_index=="Precios":
                ...
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.plotly_chart(line_plotly(table_dis.iloc[12:,:], table_dis.columns.tolist(), "Evolució trimestral dels preus per m\u00b2 construït por tipología d'habitatge", "€/m2 útil", True), use_container_width=True, responsive=True)
                with right_col:
                    st.plotly_chart(bar_plotly(table_dis_y, table_dis.columns.tolist(), "Evolució anual dels preus per m\u00b2 construït por tipología d'habitatge", "€/m2 útil", 2017), use_container_width=True, responsive=True)
            if selected_index=="Superficie":
                min_year=2014
                ...
                left_col, right_col = st.columns((1,1))
                with left_col:
                    st.plotly_chart(line_plotly(table_dis.iloc[12:,:], table_dis.columns.tolist(), "Evolució trimestral de la Superficie media por tipología d'habitatge", "m2 útil", True), use_container_width=True, responsive=True)
                with right_col:
                    st.plotly_chart(bar_plotly(table_dis_y, table_dis.columns.tolist(), "Evolució anual de la Superficie media por tipología d'habitatge", "m2 útil", 2017), use_container_width=True, responsive=True)
            if selected_index=="Definición de producto":
                def bar_plotly(table_n, selection_n, title_main, title_y, year_ini, year_fin=datetime.now().year-1):
                    table_n = table_n.reset_index()
                    table_n["Any"] = table_n["Any"].astype(int)
