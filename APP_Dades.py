...
if selected=="Edificabilidad":
    ...
    def bar_plotly(df, title_main, title_y, bar_color):
        ...
        fig.update_layout(
            title=dict(text=title_main, font=dict(size=15)),
            xaxis=dict(title=""),
    ...
    left, center, right= st.columns((1,1,1))
    with left:
        st.plotly_chart(bar_plotly(super1_df, "", "Superficie en m\u00b2", "#deffa0"), use_container_width=True, responsive=True)
    with center:
        st.plotly_chart(bar_plotly(super2_df, "", "Superficie en m\u00b2", "#9EA1D4"), use_container_width=True, responsive=True)
    with right:
        st.plotly_chart(bar_plotly(super3_df, "", "Superficie en m\u00b2", "#F6B974"), use_container_width=True, responsive=True)
    st.markdown(f'<div class="custom-box">MIX DE VIVIENDAS SEGÚN NÚMERO DE HABITACIONES</div>', unsafe_allow_html=True)   
    left, center, right= st.columns((1,1,1))
    with left:
        st.plotly_chart(donut_plotly(mix_viviendas1_df, ""), use_container_width=True, responsive=True)
    with center:
        st.plotly_chart(donut_plotly(mix_viviendas2_df, ""), use_container_width=True, responsive=True)
    with right:
        st.plotly_chart(donut_plotly(mix_viviendas3_df, ""), use_container_width=True, responsive=True)
...
if selected == "Análisis estático":
    ...
    if selected_propuesta!="Comparativa":
        ...
        info_mun = pd.read_excel("Propuestas promo.xlsx", sheet_name = "Edificabilidad1")
...
if selected == "Análisis dinámico":
    ...
    if selected_propuesta=="Comparativa":
        ...
        st.markdown(f'<div class="custom-box">TASA DE DESCUENTO Y VALOR ACTUAL NETO</div>', unsafe_allow_html=True) 
        left, center, right= st.columns((1,1,1))
        with left:
...
if selected == "Resumen de resultados":
    # IMPORTAR RESULTADOS FINALES (EXCEL EDIFICABILIDAD EN LOCAL)
    propuesta_estatico_df = []
    propuesta_dinamico_df = []
    propuesta_edificabilidad_df = []

    for i in ["Edificabilidad1", "Edificabilidad2", "Edificabilidad3"]:
        df_imported = pd.read_excel("Propuestas promo.xlsx", sheet_name=i)
        df_imported = df_imported.dropna(how="all")
        propuesta_edificabilidad_df.append(df_imported)
    for i in ["Propuesta_est1", "Propuesta_est2", "Propuesta_est3"]:
        with st.spinner('Procesando, por favor espere...'):
            conn = st.connection("gsheets", type=GSheetsConnection)
            df_imported = conn.read(worksheet=i, usecols=list(range(3)), ttl=5)
            df_imported = df_imported.dropna(how="all")
            propuesta_estatico_df.append(df_imported)
    for i in ["Propuesta_din1", "Propuesta_din2", "Propuesta_din3"]:
        with st.spinner('Procesando, por favor espere...'):
            conn = st.connection("gsheets", type=GSheetsConnection)
            df_imported = conn.read(worksheet=i, usecols=list(range(11)), ttl=5)
            df_imported = df_imported.dropna(how="all")
            propuesta_dinamico_df.append(df_imported)
    ...
    def filedownload_propuestas(filename, num_propuesta):
        ...
        <button class="button-propuesta{num_propuesta}">Descargar</button></a>"""
        return href
    left, center, right= st.columns((1,1,1))
    with left:
        st.markdown(filedownload_propuestas(f"Resumen propuesta 1.xlsx", 1), unsafe_allow_html=True)
    with center:
        st.markdown(filedownload_propuestas(f"Resumen propuesta 2.xlsx", 2), unsafe_allow_html=True)
    with right:
        st.markdown(filedownload_propuestas(f"Resumen propuesta 3.xlsx", 3), unsafe_allow_html=True)
...
def table_year(data_ori, year_ini, rounded=False, formated=True):
    ...
    if rounded==True:
        numeric_columns = data_ori.select_dtypes(include=['float64', 'int64']).columns
        data_ori[numeric_columns] = data_ori[numeric_columns].applymap(lambda x: round(x, 1))
    data_output = data_ori[(data_ori["Año"]<=str(2023)) & (data_ori["Año"]>=str(year_ini))].T
...
existing_data = pd.read_excel("Propuestas promo.xlsx", sheet_name="Edificabilidad1")
